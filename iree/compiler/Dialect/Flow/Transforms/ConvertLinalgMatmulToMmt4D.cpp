...
struct ConvertMatmulToMmt4DTargetInfo {
...
// Current instances of integer matmuls in linalg tend to use signless integers
// to mean signed. Accept both, while this is sorted out.
static bool isSignedOrSignlessInteger(Type type, int width) {
  return type.isSignedInteger(width) || type.isSignlessInteger(width);
}

class Mmt4DTileParams {
public:
  Mmt4DTileParams(int64_t M0, int64_t K0, int64_t N0, std::string comment) : M0(M0), K0(K0), N0(N0), comment(comment) {}
...
static Mmt4DTileParams chooseTileParamsForElemTypesAndTargetInfo(Type lhsElemType, Type rhsElemType, Type accElemType, const ConvertMatmulToMmt4DTargetInfo& target_info)
{
  if (isSignedOrSignlessInteger(lhsElemType, 8) &&
      isSignedOrSignlessInteger(rhsElemType, 8) &&
      isSignedOrSignlessInteger(accElemType, 32) &&
      target_info.aarch64 &&
      target_info.dotprod) {
    return Mmt4DTileParams(8, 4, 8, "i8*i8->i32, aarch64, dotprod");
  }
  return Mmt4DTileParams(8, 2, 4, "generic tiling parameters, as no known kernel was matched for this matmul and target");
}

static Mmt4DTileParams chooseTileParams(Value lhs, Value rhs, Value acc, const ConvertMatmulToMmt4DTargetInfo& target_info)
{
  ...
  return chooseTileParamsForElemTypesAndTargetInfo(lhsElemType, rhsElemType, accElemType, target_info);
  // TODO: selecting special narrow kernels on narrow static accumulator shapes.
}
...
class LinalgMatmulOpToLinalgMmt4DOpPattern
 ...
 public:
  LinalgMatmulOpToLinalgMmt4DOpPattern(MLIRContext *context, const ConvertMatmulToMmt4DTargetInfo& target_info)
  ...
  LogicalResult matchAndRewrite(linalg::MatmulOp matmulOp,
    ...
    Mmt4DTileParams tile_params = chooseTileParams(lhs, rhs, acc, target_info);
    ...
    mmt4d->setAttr(StringAttr::get(getContext(), "comment"), StringAttr::get(getContext(), tile_params.getComment()));
 ...
 private:
  ConvertMatmulToMmt4DTargetInfo target_info;
};
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    // Main pattern.
    {
      ...
      patterns.insert<LinalgMatmulOpToLinalgMmt4DOpPattern>(context, target_info);
