func.func @foo(%lhs: tensor<?x?x8x4x16x2x4xbf16>, %rhs: tensor<?x?x4x2x4x16x2x4xbf16>) -> tensor<?x?x8x4x2x4x16x4xf32> {
  %c0 = arith.constant 0 : index
  %c0_f32 = arith.constant 0.0 : f32
  %m = tensor.dim %lhs, %c0 : tensor<?x?x8x4x16x2x4xbf16>
  %n = tensor.dim %rhs, %c0 : tensor<?x?x4x2x4x16x2x4xbf16>
  ...
  %result = iree_gpu.multi_mma %lhs, %rhs, %zero {
    indexing_maps = [
      ...
      intrinsic =  MFMA_F32_16x16x16_BF16,
      unroll_m = 8,
      unroll_n = 2,
      unroll_n_to_subgroups = 4,
      unroll_k = 2
    >} : tensor<?x?x8x4x16x2x4xbf16>, tensor<?x?x4x2x4x16x2x4xbf16> into tensor<?x?x8x4x2x4x16x4xf32>
